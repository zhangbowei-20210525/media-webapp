...
export class CopyrightsService {
  ...
  getCopyrightAreaOptions() {
    return this.http.get<ResponseDto<any[]>>('/api/v1/rights/area_numbers');
  }

  getCustomerWithLikeKeyword(keyword: string) {
    return this.http.get<ResponseDto<{ id: number, name: string, contact_name: string }[]>>(
      '/api/v1/customer/search', { params: { keyword: keyword } });
  }

  getSeriesWithLikeKeyword(keywords: string) {
    return this.http.get<ResponseDto<{ id: number, series_name: string, series_type: string, episodes_num: number }[]>>(
  ...
  addCopyrights(contract: ContractCopyrightDto) {
    return this.http.post<ResponseDto<any>>('/api/v1/rights', contract);
  }

  purchaseCopyrights(contract: ContractCopyrightDto) {
    return this.http.post<ResponseDto<any>>('/api/v1/rights?type=purchase', contract);
  }

  publishCopyrights(contract: ContractCopyrightDto) {
    return this.http.post<ResponseDto<any>>('/api/v1/publish_rights', contract);
  }

  homemadeCopyrights(contract: ContractCopyrightDto) {
    return this.http.post<ResponseDto<any>>('/api/v1/rights?type=homemade', contract);
  }
  ...
  deleteCopyright(id: number) {
    return this.http.delete<ResponseDto<any>>(`/api/v1/rights/${id}`);
  }

  getOwnCopyrightDetailsInfo(id: number) {
    return this.http.get<ResponseDto<any>>(`/api/v1/rights/${id}`);
  }

  getPubCopyrightDetailsInfo(pagination: PaginationDto, id: number) {
    return this.http.get<ResponseDto<any>>(`/api/v1/publish_rights/${id}?page=${pagination.page}&page_size=${pagination.page_size}`);
  }

  deleteCopyrightDetailsInfo(id: number) {
    return this.http.delete<ResponseDto<any>>(`/api/v1/rights_detail/${id}`);
  }
  ...
  getCopyrightsType() {
    return this.http.get<ResponseDto<any[]>>('/api/v1/rights/template/right_types');
  }
}
