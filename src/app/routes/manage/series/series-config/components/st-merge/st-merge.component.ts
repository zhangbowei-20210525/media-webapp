...
@Component({
  selector: 'app-st-merge',
  templateUrl: './st-merge.component.html',
  styles: [`
    .merge-tag {
      position: relative;
      margin: 5px;
...
export class StMergeComponent implements OnInit {
  ...
  handleClose(event, info: any): void {
    ...
    this.modal.confirm({
      ...
      nzCancelText: '我再想想',
      nzOkType: 'danger',
      nzOnOk: () => this.service.deleteType('program_types', info.id).subscribe(result => {
        this.refresh();
      }),
      nzOnCancel: () => {
        // this.refresh();
  ...
  mergeAgreed(a: any, b: any) {
    this.service.typeMerge('program_types', a.id, b.raw).subscribe(result => {
      this.refresh();
    });
  }
