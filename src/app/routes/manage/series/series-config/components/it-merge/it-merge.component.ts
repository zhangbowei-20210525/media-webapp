...
@Component({
  selector: 'app-it-merge',
  templateUrl: './it-merge.component.html',
  styles: [`
    .merge-tag {
      position: relative;
      margin: 5px;
...
export class ItMergeComponent implements OnInit {
  ...
  handleClose(event, info: any): void {
    ...
    this.modal.confirm({
      ...
      nzCancelText: '我再想想',
      nzOkType: 'danger',
      nzOnOk: () => this.service.deleteType('investment_types', info.id).subscribe(result => {
        this.refresh();
      }),
  ...
  mergeAgreed(a: any, b: any) {
    this.service.typeMerge('investment_types', a.id, b.raw).subscribe(result => {
      this.refresh();
    });
  }
